...
func (s *Store) ListAllowedOrigins(ctx context.Context) ([]AllowedOrigin, error) {
	rows, err := s.pool.Query(ctx, `select id, owner_id, origin, label, enabled, created_at from allowed_origins order by created_at asc`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var o AllowedOrigin
		if err := rows.Scan(&o.ID, &o.OwnerID, &o.Origin, &o.Label, &o.Enabled, &o.CreatedAt); err != nil {
			return nil, err
		}
		out = append(out, o)
	}
...
func (s *Store) ListAllowedOriginsForOwner(ctx context.Context, ownerID string) ([]AllowedOrigin, error) {
	rows, err := s.pool.Query(ctx, `select id, owner_id, origin, label, enabled, created_at from allowed_origins where owner_id = $1 order by created_at asc`, ownerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
insert into allowed_origins (owner_id, origin, label) values ($1, $2, $3)
returning id, owner_id, origin, label, enabled, created_at`, ownerArg, origin, strings.TrimSpace(label))
	err := row.Scan(&o.ID, &o.OwnerID, &o.Origin, &o.Label, &o.Enabled, &o.CreatedAt)
	return o, err
}
...
where id = $1
returning id, owner_id, origin, label, enabled, created_at`, id, strings.TrimSpace(label), enabled)
	err := row.Scan(&o.ID, &o.OwnerID, &o.Origin, &o.Label, &o.Enabled, &o.CreatedAt)
...
returning id, owner_id, origin, label, enabled, created_at`, id, ownerID, strings.TrimSpace(label), enabled)
	err := row.Scan(&o.ID, &o.OwnerID, &o.Origin, &o.Label, &o.Enabled, &o.CreatedAt)
	if errors.Is(err, pgx.ErrNoRows) {
		return o, ErrNotFound
	}
